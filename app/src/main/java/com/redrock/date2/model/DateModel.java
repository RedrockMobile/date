...
import com.redrock.date2.model.bean.School;
...
public class DateModel extends AbsModel {
    ...
    //获取收藏
    public void getCollection(int page,DataCallback<Date[]> callback) {
        TokenParams params = new TokenParams();
        params.put("page",page+"");
    ...
    //获取参加的约
    public void getJoinDate(int page,DataCallback<Date[]> callback){
        TokenParams params = new TokenParams();
        params.put("page",page+"");
    ...
    public DateTypeFather[] getDateTypeFather() {
        return (DateTypeFather[]) JFileManager.getInstance().getFolder(Dir.Object).readObjectFromFile(DATE_TYPE_FILE);
    }
    ...
    public void publishDate(DateEdit dateEdit, StatusCallback callback) {
        ...
        params.put("shcool_limit", dateEdit.getSchool());
    ...
    public int getFiltrateStyle() {
        return JUtils.getSharedPreference().getInt(FILTRATE_STYLE, 0);
    }

    public int getFiltrateUser() {
        return JUtils.getSharedPreference().getInt(FILTRATE_USER, 0);
    }

    public int getFiltrateTime() {
        return JUtils.getSharedPreference().getInt(FILTRATE_TIME, 0);
    }

    public int getFiltrateCost() {
        return JUtils.getSharedPreference().getInt(FILTRATE_COST, 0);
    }

    //获取学校
    public void getSchool() {
        RequestManager.getInstance().post(API.URL.getSchool, null, new DataCallback<School[]>() {
            @Override
            public void success(String info, School[] data) {
                saveSchool(data);
            }
        });
    }

    public void saveSchool(School[] data) {
        JFileManager.getInstance().getFolder(Dir.Object).writeObjectToFile(data,SCHOOL);
    }

    public School[] getSchoolFromFile() {
        return (School[]) JFileManager.getInstance().getFolder(Dir.Object).readObjectFromFile(SCHOOL);
    }

    public void getDateList(int dateType, int page, DataCallback<Date[]> callback) {
        TokenParams params = new TokenParams();
        params.put("dateType", dateType + "");
        params.put("page", page + "");
        RequestManager.getInstance().post(API.URL.getDateList, params, callback);
    }
