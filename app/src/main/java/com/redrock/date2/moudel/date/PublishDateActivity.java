...
@RequiresPresenter(PublishDatePresenter.class)
public class PublishDateActivity extends BaseActivity<PublishDatePresenter> {
    ...
    public void showSchool() {
        if (DateModel.getInstance().getSchoolFromFile() == null || DateModel.getInstance().getSchoolFromFile().length==0) {
            return;
        }
        School[] data = DateModel.getInstance().getSchoolFromFile();
        String[] school = new String[data.length];
        for (int i = 0; i < data.length; i++) {
            school[i] = data[i].getName();
        }
        new MaterialDialog.Builder(this)
                .title("选择学校")
                .items(school)
                .itemsCallbackSingleChoice(0, new MaterialDialog.ListCallbackSingleChoice() {
                    ...
                    public boolean onSelection(MaterialDialog dialog, View view, int which, CharSequence text) {
                        ...
                        getPresenter().setSchool(text.toString());
                        return true;
                    }
                })
                .positiveText("确定")
